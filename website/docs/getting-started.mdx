...
supported arrangement is pairmux inside WSL. This finds WSL, checks it has a distribution, and runs
the POSIX installer inside it rather than pretending a Windows install is possible:
...
`PAIRMUX_WSL_DISTRO`, `PAIRMUX_INSTALL_DIR`. pairmux then lives inside the distribution: run it as
`wsl -- pairmux version`, and install tmux there too if it is not already present.
